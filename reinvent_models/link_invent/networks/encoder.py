...
class Encoder(tnn.Module):
    ...
    def _initialize_hidden_state(self, batch_size: int) -> torch.Tensor:
        return torch.zeros(self.num_layers*2, batch_size, self.num_dimensions).cuda() if torch.cuda.is_available() else torch.zeros(self.num_layers*2, batch_size, self.num_dimensions)
